...
class Attention(tf.keras.layers.Layer):
    ...
    def call(self, query, key):
        ...
        score = self.V(tf.nn.tanh(self.W_query(query_with_time_axis) + self.W_key(key)))
...
class AttRouteNet_Fermi(tf.keras.Model):
    def __init__(self):
        super(AttRouteNet_Fermi, self).__init__()
        ...
        # Attention layers
        self.attention_path_to_queue = Attention(32)
        self.attention_queue_to_link = Attention(32)
        ...
        # Gated Recurrent Unit Cells used in Message Passing to update path, link, and queue states 
    ...
    def call(self, inputs):
        ...
        for it in range(self.iterations):
            ...
            attended_path_to_queue, _ = self.attention_path_to_queue(path_state_sequence, queue_state)
            attended_queue_to_link, _ = self.attention_queue_to_link(queue_state, link_state)

            ###################
            #  PATH TO QUEUE  #
            ###################
            queue_state, _ = self.queue_update(attended_path_to_queue, [queue_state])


            ###################
            #  QUEUE TO LINK  #
            ###################
            link_state = link_gru_rnn(attended_queue_to_link, initial_state=link_state)
